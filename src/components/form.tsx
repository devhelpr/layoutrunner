...
export class FormComponent extends React.Component<FormProps, FormState> {
			...
			layoutBlock.form.map((formField : any) => {
				if (formField.layout) {
					if (formField.type == "layout") {
						result = result.concat(this.getAdditionalStateFields(formField.layout));
					} else {
	...
	componentDidMount() {
		...
		if (!this.props.hasParentForm) {
			...
			if (this.props.formDefinition) {
				...
				const formFieldNames = this.getAdditionalStateFields(this.props.layoutBlock);
	...
	render() {
		...
		return <>
			<FormHeader hasParentForm={this.props.hasParentForm} caption={this.props.caption}></FormHeader>
			<FormBody {...this.state} hasParentForm={this.props.hasParentForm}>{this.props.formDefinition && 
				this.props.formDefinition.map((formField : any, index) => {
					...
					return <React.Fragment key={index}>
							{this.props.renderFormControl && this.props.renderFormControl(
